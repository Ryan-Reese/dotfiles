...
- Run `conda-init` (defined in `~/.zshrc`) before using `conda` — it is not in PATH by default
- Prefer `uv` for creating and managing virtual environments
- ALWAYS perform linting using `ruff` before committing changes
- ALWAYS perform testing using `pytest` before committing changes

Quick reference:
- Init conda: `conda-init` (required once per shell session)
